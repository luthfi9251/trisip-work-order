import { GetSummaryProductUseCase } from "@/lib/application/use-cases/work-orders/get-summary-product.use-case";
...
const getSummaryProductUseCase = new GetSummaryProductUseCase(
    userRepository,
    workOrderRepository
);
...
export const getSummaryProductController = async (user_id: string) => {
    ...
    const result = await getSummaryProductUseCase.execute(user_id);
    return result;
};
